...
| 6 | Generate new decal artwork | **PASS**, re-validated live | **28** real GPU generations (25 research + 1 M7 review + 1 M8 deployment validation + 1 M11 final-audit validation); `docs/evidence/prototype-5/`, `EXP-031`, `EXP-033`. **M11 ran one authorised generation on 2026-08-15** and it reproduced the expected PNG **byte-identically** — `46bbf160e427…`, 1 089 939 B — with the hash declared before the run (`gpu-validation.md`) | §13 | slide 9 | — |
...
| 8 | Provide a reproducible deployment or demonstration setup | **PASS** | `docs/deployment/runbook.md` + `weights-manifest.md`; **M11 clean clone re-run today**: fresh venv, `npm ci`, documented restore, 3/3 adapters SHA-256 verified, backend serving, built frontend serving, 522 passed / 5 skipped | §16 | slide 12 | **cleared** — `preflight.ps1` returned **10 of 10 PASS** on 2026-08-15 once port 8000 was freed. The holder was **not** Docker Desktop itself but an unrelated project's container (`aegislab-api-1`); it was stopped, then restarted afterwards |
