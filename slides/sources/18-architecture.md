<svg class="diagram" viewBox="0 0 1440 330" role="img" aria-label="Two processes: a React frontend calling a single-worker FastAPI service that holds one resident diffusion pipeline on the GPU">
  ...
  <g font-family="Segoe UI, sans-serif">
    ...
    <text x="4" y="296" font-size="22" fill="#46525f">Two processes, one machine. Single-worker is a correctness requirement enforced in code, not a deployment convenience &#8212; a second resident pipeline does not fit.</text>
  </g>
</svg>

## Speaker notes

Briefly, because the interesting part is a constraint rather than a diagram.

Two processes on one machine: a React frontend with the Three.js deck, and a FastAPI service that
owns the model.

The single worker is what I would point at. It is not a default I never changed — it is asserted in
code at startup, because a second resident pipeline does not fit. Two simultaneous requests, and
the second gets a clean refusal rather than an out-of-memory crash that takes the first down with
it. Refusing correctly is a feature here.

Uploads are validated properly — allowlist, real decode, size limits, random internal filenames, no
user string reaching a path. An upload endpoint attached to a GPU is worth attacking.

The deck geometry is procedural, so no third-party model licence enters the deliverable.
