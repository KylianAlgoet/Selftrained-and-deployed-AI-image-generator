...
- **Inference is deterministic.** A clean clone reproduced an earlier output **byte for byte**, three days later, in a freshly built environment
- **Training is not.** The adapter initialisation draws from an **unseeded global generator**, so a run cannot be repeated from its recorded seed
- So the checkpoints are **artifacts, verified by hash** — not recipes
...
environment, three days later, reproduced an earlier output byte for byte. Same SHA-256.
...
So the checkpoints are artifacts, not recipes: verified by hash on every request, and the
deployment docs say plainly that three required files cannot be regenerated.

The part worth pushing on: that defect produces runs that look completely fine. It is invisible
unless you compare weights — which I only did because I tried to reproduce my own result.
